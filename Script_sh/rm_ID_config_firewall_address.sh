...
ARQUIVO_EXTRAIDO="$PASTA_SAIDA/config_firewall_address.cfg"
ARQUIVO_FILTRADO="$PASTA_SAIDA/no_ID_config_firewall_address.cfg"
...
def extrair_enderecos(arquivo):
    ...
    pattern = r"config firewall address\n(.*?)\nend"
    enderecos = []
